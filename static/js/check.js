document.addEventListener('DOMContentLoaded', function () {
    ...
    let ageValue = document.querySelector('.slide-value');
    let genderInputs = document.querySelectorAll('.info2 .form input[name="gender"]');
    let checkboxForms = document.querySelectorAll('.info3 form');
    ...
    next.onclick = function () {
        if (active === 0 && !checkbox.checked) {
            alert("Please agree to the terms and conditions first.");
            return;
        } else if (active === 1 && ageSlider.value === '0') {
            alert("Please select your age.");
            return;
        } else if (active === 1 && !isGenderSelected()) {
            alert("Please select your gender.");
            return;
        } else if (active === 2 && !areCheckboxesChecked()) {
            alert("Please check all the statements before proceeding.");
            return;
        }
    ...
    function filterSymptoms(query) {
        const filteredSymptoms = symptoms.filter(symptom => {
            return symptom.toLowerCase().includes(query.toLowerCase());
        });
        populateSymptoms(filteredSymptoms);
    }
    ...
    function populateSymptoms(symptomsArray = symptoms) {
        ...
        symptomsArray.forEach(symptom => {
            ...
            scrollDiv.appendChild(symptomDiv);
            symptomDiv.addEventListener('click', () => {
                moveSymptom(symptomDiv, scrollDiv, selectedDiv);
            });
        });
    }
    function moveSymptom(symptomDiv, sourceDiv, targetDiv) {
        targetDiv.appendChild(symptomDiv);
        const symptoms = Array.from(targetDiv.querySelectorAll('.symptom'));
        symptoms.sort((a, b) => {
            return a.textContent.localeCompare(b.textContent);
        });
        targetDiv.innerHTML = '';
        symptoms.forEach(symptom => {
            targetDiv.appendChild(symptom);
        });
    }
    populateSymptoms();
    ...
    selectedDiv.addEventListener('click', (event) => {
        if (event.target.classList.contains('symptom')) {
            moveSymptom(event.target, selectedDiv, scrollDiv);
        }
    });
    scrollDiv.addEventListener('click', (event) => {
        if (event.target.classList.contains('symptom')) {
            moveSymptom(event.target, scrollDiv, selectedDiv);
        }
    });
    ...
    function updateDiseases() {
        ...
        Object.entries(Diseases).forEach(([disease, symptoms]) => {
            const commonSymptoms = symptoms.filter(symptom => selectedSymptoms.includes(symptom));
            const count = commonSymptoms.length;
            const possibility = ((count / totalSelectedSymptoms) * 100).toFixed(2);
        ...
        const diseaseBox = document.querySelector('.disease_box');
        diseaseBox.innerHTML = '';
        Object.entries(selectedDiseases).forEach(([disease, data]) => {
            const diseaseSymptoms = data.symptoms;
            const possibility = ((data.count / symptoms.length) * 100).toFixed(2);
            const diseaseDiv = document.createElement('div');
            diseaseDiv.classList.add('disease');
            diseaseDiv.innerHTML = `
                <div class="name">Disease: ${disease}</div>
                <div class="possibility">Possibility: ${possibility}%</div>
                <div class="symptoms">Symptoms: ${diseaseSymptoms.join(', ')}</div>
                <div class="line"></div>
            `;
            diseaseBox.appendChild(diseaseDiv);
        });
        const selectedDisease = document.querySelector('.disease_box .disease');
        if (selectedDisease) {
            const symptomsTab = document.querySelector('.symptoms');
            const symptomsList = selectedDisease.querySelector('.symptoms').textContent;
            symptomsTab.innerHTML = `<p>${symptomsList}</p>`;
        }
    }
    ...
    const symptomDivs = document.querySelectorAll('.scroll .symptom');
    symptomDivs.forEach(symptomDiv => {
        symptomDiv.addEventListener('click', () => {
            handleSymptomSelection(symptomDiv.textContent);
        });
    });
    const Diseases = {
    ...
    ageSlider.addEventListener('input', () => {
        ...
        ageValue.innerText = ageSliderValue;
    });
    ...
    function areCheckboxesChecked() {
        for (let i = 0; i < checkboxForms.length; i++) {
            let inputs = checkboxForms[i].querySelectorAll('input[type="radio"]');
            let checked = false;
